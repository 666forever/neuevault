...
export function createPages(repository, app, openAsset) {
  ...
  function home() {
    ...
    app.innerHTML = `<div class="page"><section class="hero"><video class="hero-video" autoplay muted loop playsinline preload="metadata" data-src-small="/assets/video/furina-hero-1080p.mp4" data-src-large="/assets/video/furina-hero-1440p.mp4" aria-hidden="true"></video><div class="hero-gradient" aria-hidden="true"></div><div class="hero-grain" aria-hidden="true"></div><div class="hero-content"><p class="hero-eyebrow">Meet pfseeker 2.0</p><h1>Discover the Best Banners on the internet. Literally.</h1><p class="hero-description">Stop digging through endless pages of repeats, trend-chasing, or whatever everyone else is already using. Browse alt, emo, dark, soft, strange, cute, messy, and the spaces where they cross. Let different aesthetics coexist. Identity forms in the borderland.</p><a class="button hero-cta" href="/recent"><span class="hero-cta-icon" aria-hidden="true"></span><span>Get Full Access</span></a></div></section>${categorySection}</div>${collectionSection}<section class="section recent-section"><div class="section-head"><div><h2>Recently Added</h2><p>The newest finds, in every format.</p></div><a class="text-link" href="/recent">Browse archive</a></div>${renderAssetGrid(assets.slice(0, 8))}</section>`;
    mount();
