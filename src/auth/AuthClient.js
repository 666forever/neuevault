export class AuthClient extends EventTarget {
  constructor(fetcher = (...args) => fetch(...args)) { super(); this.fetcher = fetcher; this.signInStarted = false; this.state = { loading: true, configured: false, authenticated: false, user: null, csrfToken: null }; }
  async load() {
    try {
  ...
  signIn(returnTo = '/') { if (this.signInStarted) return; this.signInStarted = true; location.assign(`/api/auth/discord?returnTo=${encodeURIComponent(returnTo)}`); }
  async logout() {
