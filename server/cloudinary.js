...
export async function protectedDownloadUrl(asset, env, now = Math.floor(Date.now() / 1000)) {
  ...
  const resourceType = asset.originalDelivery?.resourceType || 'image'; if (resourceType !== 'image') return null;
  const params = { attachment: 'true', expires_at: String(now + 300), format: asset.fileType.toLowerCase(), public_id: asset.cloudinaryPublicId, timestamp: String(now), type: 'authenticated' };
  ...
  return `https://api.cloudinary.com/v1_1/${encodeURIComponent(env.CLOUDINARY_CLOUD_NAME)}/${resourceType}/download?${query}`;
}
