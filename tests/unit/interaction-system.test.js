...
describe('shared interaction system', () => {
  it('defines one tokenized rolling-control primitive with accessible duplicate layers', async () => {
    ...
    for (const token of ['--duration-link-roll: 300ms', '--duration-nav-pill: 150ms', '--delay-hover-intent: 10ms', '--ease-link-roll: cubic-bezier(0.76, 0, 0.24, 1)', '--roll-travel: 40px', '--roll-axis: 49% 50%', '--nav-container-max: 1440px', '--nav-item-height: 40px', '--nav-item-padding-inline: 15px', '--nav-item-gap: 2px', '--nav-pill-bg-hover: #1a1a1a', '--nav-pill-bg-active: #151515', '--nav-action-height: 38px', '--nav-motion-duration: 220ms']) expect(css).toContain(token);
    ...
    expect(css).toMatch(/\.main-nav a::before\s*\{[\s\S]*?inset:\s*0[\s\S]*?background:\s*var\(--nav-pill-surface, var\(--nav-pill-bg-hover\)\)[\s\S]*?transition:\s*opacity var\(--nav-pill-duration\) var\(--ease-standard\)/);
