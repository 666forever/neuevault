...
const roots = ['src', 'dist'].map(directory => path.join(projectRoot, directory)); const forbidden = ['CLOUDINARY_API_SECRET', 'CLOUDINARY_API_KEY=', 'api_secret', 'GITHUB_APP_PRIVATE_KEY', 'GITHUB_APP_INSTALLATION_ID', 'CLOUDFLARE_PAGES_READ_TOKEN']; const violations = [];
...
else console.log('Server-provider browser-bundle secret audit passed.');
