...
import { readJson, walkFiles } from './filesystem.mjs';
import { resolveLocalAssets } from './local-adapter.mjs';
...
export async function generateAssets({ config = pipelineConfig, writeOutput = true, writeManifests = true, clean = false } = {}) {
  ...
  const resolvedAssets = await resolveLocalAssets(authoredFile.assets, config, report, false); report.assertValid();
  let compiled;
  ...
  if (writeOutput) {
    ...
    const expectedPreviews = new Set(assets.map(asset => path.resolve(config.publicPreviewRoot, path.basename(asset.previewFile))));
    ...
    await resolveLocalAssets(authoredFile.assets, config, report, true); report.assertValid();
    if (writeManifests) {
